...
def actor(agent: DrQAgent, data_store, env, sampling_rng):
    ...
    # Initialize reward model and queues
    reward_model_path = "reward_model/reward_model3.pt"
    if not os.path.exists(reward_model_path):
        raise FileNotFoundError(f"Reward model not found at: {reward_model_path}. Please check the path.")
    
    # Use auto to allow GPU usage if available
    reward_model = load_pytorch_model(reward_model_path, device="auto")
    ...
    for step in tqdm.tqdm(range(FLAGS.max_steps), dynamic_ncols=True):
        ...
        with timer.context("step_env"):
            ...
            ############################################################
            # Update reward using history queue
            ############################################################

            curr_base = process_image(next_obs['front'])
            curr_hand = process_image(next_obs['wrist'])
            curr_state = process_state(next_obs['state'])
            ...
            with timer.context("reward_model_inference"):
                with torch.no_grad():
            ...
            running_env_return += float(env_reward)
            ...
            if done or truncated:
                episode_returns.append(float(running_return))
                stats = {
                    "train/episode_return": float(running_return),
                    "train/env_episode_return": float(running_env_return),
                }
        ...
        if step % FLAGS.log_period == 0:
            ...
            if step_rewards:
                stats["train/step_reward"] = np.mean(step_rewards)
                step_rewards = []
            if len(episode_returns) > 0:
                stats["train/episode_return"] = np.mean(episode_returns)
            client.request("send-stats", stats)
